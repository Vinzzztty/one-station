import Link from "next/link";
import { getAllArticlesForAdmin } from "@/services/blogs.service";
import DeleteArticleButton from "@/components/ui/DeleteArticleButton";

export default async function ArticlesPage() {
  const articles = await getAllArticlesForAdmin();

  return (
    <div className="space-y-6">
      {/* HEADER */}
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold text-foreground">Articles</h2>
          <p className="text-muted mt-1">
            Manage your blog posts and news here.
          </p>
        </div>
        <Link
          href="/contents/articles/create"
          className="bg-primary hover:bg-primary-dark text-white px-5 py-2.5 rounded-lg font-medium shadow-sm transition-colors flex items-center gap-2 hover-lift"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="18"
            height="18"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          >
            <line x1="12" y1="5" x2="12" y2="19"></line>
            <line x1="5" y1="12" x2="19" y2="12"></line>
          </svg>
          Add New Article
        </Link>
      </div>

      {/* TABLE CARD */}
      <div className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border bg-background/50 text-muted text-sm uppercase tracking-wider">
              <th className="p-4 font-semibold">Title</th>
              <th className="p-4 font-semibold">Author</th>
              <th className="p-4 font-semibold text-right">Actions</th>
            </tr>
          </thead>

          <tbody className="divide-y divide-border">
            {articles.map((item) => (
              <tr
                key={item.id}
                className="group hover:bg-background transition-colors"
              >
                <td className="p-4">
                  <div className="font-medium text-foreground">
                    {item.title}
                  </div>
                </td>

                <td className="p-4 text-muted text-sm">
                  {item.admin?.name ?? "Admin"}
                </td>

                <td className="p-4 text-right">
                  <div className="flex justify-end gap-2">
                    {/* EDIT - Menggunakan Link (Navigasi) */}
                    <Link
                      href={`/contents/articles/${item.id}/edit`}
                      className="p-2 text-muted hover:text-primary transition-colors rounded-md hover:bg-primary/10"
                      title="Edit"
                    >
                      ✏️
                    </Link>

                    {/* DELETE - Menggunakan Client Component */}
                    <DeleteArticleButton id={item.id} />
                  </div>
                </td>
              </tr>
            ))}

            {/* Handling jika data kosong */}
            {articles.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-muted">
                  No articles found. Create one to get started.
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>

      {/* Pagination (statik dulu, UI tetap) */}
      <div className="flex justify-between items-center text-sm text-muted pt-2">
        <span>
          Showing 1-{articles.length} of {articles.length} articles
        </span>
        <div className="flex gap-2">
          <button
            className="px-3 py-1 border border-border rounded hover:bg-surface disabled:opacity-50"
            disabled
          >
            Previous
          </button>
          <button className="px-3 py-1 border border-border rounded hover:bg-surface">
            Next
          </button>
        </div>
      </div>
    </div>
  );
}
